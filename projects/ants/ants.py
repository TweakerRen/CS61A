        ...
        if self.doubled == False:
            if self.is_container:
                ...
                if self.ant_contained: 
                    if not self.ant_contained.doubled and hasattr(self.ant_contained, 'damage'):
                        self.ant_contained.damage *= 2
                        self.ant_contained.doubled = True
            elif hasattr(self, 'damage'):
                self.damage *= 2
                self.doubled = True
        if self.doubled == True and self.is_container and self.ant_contained:
...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self):
        ...
        count = 0
        while True:
            if inspection.is_hive:
                return None
            if self.lower_bound <= count and self.upper_bound >= count:
                if inspection.bees:
                    return random_bee(inspection.bees)
...
class FireAnt(Ant):
    ...
    def reduce_health(self, amount):
        ...
        if self.place:
            ...
            for bee in the_place.bees.copy():
    ...
    def action(self, gamestate):
        if self.turns_to_chew == 0:
            if random_bee(self.place.bees):
                ...
                Ant.reduce_health(select_bee, select_bee.health)
...
class QueenAnt(ThrowerAnt):
    ...
    def action(self, gamestate):
        ...
        while current_place.exit: 
            if current_place.exit.ant:
                current_place.exit.ant.double()
...
class NinjaAnt(Ant):
    ...
    def action(self, gamestate):
        # BEGIN Problem Optional 1
        for each_bee in self.place.bees.copy():
            each_bee.reduce_health(self.damage)
        # END Problem Optional 1
...
class LaserAnt(ThrowerAnt):
    ...
    def insects_in_front(self):
        ...
        while not current_place.is_hive:
            for bee in current_place.bees:
                insect_distance[bee] = count
            if current_place.ant and current_place.ant != self:
                insect_distance[current_place.ant] = count
    ...
    def calculate_damage(self, distance):
        # BEGIN Problem Optional 2
        result = 2 - distance * 0.25 - self.insects_shot * 0.0625
        if result >= 0 :
            return result
        else:
            return 0
